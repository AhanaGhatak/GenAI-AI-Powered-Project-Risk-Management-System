...
st.markdown("""
    ...
    <div class="main-header">
        ...
        <p>Stable Multi-Agent Architecture • 2026 Edition</p>
    </div>
...
# --- 2. VECTOR ENGINE (FIXED MODEL ID) ---
@st.cache_resource
def initialize_system():
    # Folder v6 ensures we don't have leftover 404/dimension errors
    persist_dir = "./risk_db_v6_stable"
    ...
    try:
        ...
        # 2026 Unified Model ID (Replaces text-embedding-004)
...
def run_agent_workflow(query, vector_db):
    try:
        ...
        return "System", f"Error in agent workflow: {str(e)}"

# --- 4. APP INTERFACE ---
with st.sidebar:
    st.header("⚙️ SYSTEM CONFIG")
    st.info("Using: models/gemini-embedding-001")
    if st.button("🚀 FULL SYSTEM WIPE"):
        st.cache_resource.clear()
        # Remove all potential old database folders
        for f in ["./risk_db_agents_v3", "./risk_db_final_v4", "./risk_db_final_v5", "./risk_db_v6_stable"]:
            if os.path.exists(f):
                shutil.rmtree(f)
        st.rerun()
